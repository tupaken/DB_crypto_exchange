...
CONSTRAINT chk_Konto_min CHECK (Kontostand >=0)); 
...
CONSTRAINT chk_Konto_Wallet_min CHECK (Kontostand >=0),
...
Datum DATE NOT NULL,
CONSTRAINT chk_Kurs_min CHECK (Wechsel_Kurs>0));
...
End_Datum DATE,
CONSTRAINT chk_Dates CHECK (Start_Datum<End_Datum),
...
CONSTRAINT chk_Betrag CHECK (Betrag>0));
...
Gewinn DECIMAL(20,10) DEFAULT 0.00 NOT NULL,
CONSTRAINT chk_Betrag_min CHECK (Mindest_Betrag>=0));
...
ALTER TABLE Investors
ADD CONSTRAINT fk_USER_ID_Investors
FOREIGN KEY (USER_ID)
REFERENCES "USER"(ID);
